                                                ...
                                                if request.form['vomit']=='no':
                                                    if request.form['covid']=='no':
                                                        if request.form['antibodies']=='no':
                                                            ...
                                                            return render_template('donationform.html',email=session['donoremail'],firstname=firstname,status=status,districts=districts,msg=msg)
                                                        else:
...
def feedback():
    if request.method=="POST":
        ...
        me=request.form['me']
        sql="INSERT INTO feedback(username,firstname,email,phonenumber,district,hospitalname,donationdate,blood,me,treatment) VALUES ('{}','{}','{}',{},'{}','{}','{}','{}','{}','{}')".format(username,firstname,email,phonenumber,district,hospitalname,donationdate,blood,me,treat)
        stmt = ibm_db.exec_immediate(conn,sql)  # do the task
        if me == "donor":
            return "<h3><center>Your feedback has submitted to the database.<a href={{ url_for('donorlogout') }}><button>Log Out</button></a></center></h3>"
        elif me == "recipient":
            return "<h3><center>Your feedback has submitted to the database.<a href={{ url_for('recipientlogout') }}><button>Log Out</button></a></center></h3>"
